...
public class FriendsActivity extends ListActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        final TwitterSession session = TwitterCore.getInstance().getSessionManager()
                .getActiveSession();
        UserAdapter adapter = new UserAdapter(FriendsActivity.this);
        setListAdapter(adapter);
        MyTwitterApiClient apiClient = new MyTwitterApiClient(session);
        apiClient.getFollowersService().listFriends(session.getUserId(), 200).enqueue(new retrofit2.Callback<ResponseBody>() {
            @Override
